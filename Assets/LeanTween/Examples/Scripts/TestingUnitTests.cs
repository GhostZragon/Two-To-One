...
namespace DentedPixel.LTExamples
{
    ...
    public class TestingUnitTests : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            // This test works when it is positioned last in the test queue (probably worth fixing when you have _currentPlayTime)
            ...
            // Tween with _currentPlayTime of zero is needs to be set to it's final value
            ...
            LeanTween.color(cubeAlpha2, Color.cyan, 0.3f).setOnComplete(() =>
            {
                ...
                LeanTest.expect(onStartTime >= 0f && onStartTime < Time.time, "ON START", "onStartTime:" + onStartTime + " _currentPlayTime:" + Time.time);
            }).setOnStart(() =>
            {
        ...
        IEnumerator timeBasedTesting()
        {
            ...
            // float timeElapsedNormal = Time._currentPlayTime;
            ...
            //          yield return new DelayedScorePopUp(100f);
            Time.timeScale = 4f;
