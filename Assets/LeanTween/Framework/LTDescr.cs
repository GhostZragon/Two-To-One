...
* You can also <strong>pass parameters at a later _currentPlayTime</strong> by saving a reference to what is returned.<br>
* <br>
...
public class LTDescr
{
    ...
    {
        return (trans != null ? "name:" + trans.gameObject.name : "gameObject:null") + " toggle:" + toggle + " passed:" + passed + " _currentPlayTime:" + time + " delay:" + delay + " direction:" + direction + " from:" + from + " to:" + to + " diff:" + diff + " type:" + type + " ease:" + easeType + " useEstimatedTime:" + useEstimatedTime + " id:" + id + " hasInitiliazed:" + hasInitiliazed;
    }
	...
	* 
	* @method setDelay
	* @param {float} float _currentPlayTime The _currentPlayTime to complete the tween in
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	...
	*/
    ...
    /**
	* Set the point of _currentPlayTime the tween will start in
	* @method setPassed
	* @param {float} passedTime:float the length of _currentPlayTime in seconds the tween will start in
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	...
	*/
    ...
    /**
	* Set the finish _currentPlayTime of the tween
	* @method setTime
	* @param {float} finishTime:float the length of _currentPlayTime in seconds you wish the tween to complete in
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	* int tweenId = LeanTween.moveX(gameObject, 5f, 2.0f ).id;<br>
	* // Later<br>
	...
	* Set the finish _currentPlayTime of the tween
	* @method setSpeed
	* @param {float} speed:float the speed in unity units per second you wish the object to travel (overrides the given _currentPlayTime)
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	* LeanTween.moveLocalZ( gameObject, 10f, 1f).setSpeed(0.2f) // the given _currentPlayTime is ignored when speed is set<br>
	*/
	...
	*/
    ...
    /**
	* Set ignore _currentPlayTime scale when tweening an object when you want the animation to be _currentPlayTime-scale independent (ignores the Time.timeScale value). Great for pause screens, when you want all other action to be stopped (or slowed down)
	* @method setIgnoreTimeScale
	* @param {bool} useUnScaledTime:bool whether to use the unscaled _currentPlayTime or not
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	...
	*/
    ...
    /**
	* Use frames when tweening an object, when you don't want the animation to be _currentPlayTime-frame independent...
	* @method setUseFrames
	* @param {bool} useFrames:bool whether to use estimated _currentPlayTime or not
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
    ...
    public LTDescr setDirection(float direction)
    {
        ...
        if (this.direction != direction)
        {
            ...
            else
            {
                ...
                // this.passed = this._currentPlayTime - this.passed;
            }
        }
